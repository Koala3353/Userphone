...
public class LogUtil
{
    ...
    public static String logCachedMessagesForwards(String title, List<MessageCache.CachedMessage> messages)
    {
        TextChannel deltc = messages.get(0).getTextChannel();
        Guild delg = deltc.getGuild();
        StringBuilder sb = new StringBuilder("-- "+title+" -- #"+deltc.getName()+" ("+deltc.getId()+") -- "+delg.getName()+" ("+delg.getId()+") --");
        for(int i=0; i<messages.size(); i++)
            appendMessage(sb, messages.get(i), messages.get(i).getAuthor());
        return sb.toString().trim();
    }
    ...
    private static void appendMessage(StringBuilder sb, MessageCache.CachedMessage m, User author)
    {
        sb.append("\r\n\r\n[")
                .append(m.getTimeCreated().format(DateTimeFormatter.RFC_1123_DATE_TIME))
                .append("] ");
